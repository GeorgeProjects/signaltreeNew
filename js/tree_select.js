var treeSelect = function(){
	...
	function drawHistogram(dataArray,datadimMode){
		...
		function pile_bars(level)
		{
			...
			.on('click',function(d,i){	
				...
				d3.selectAll("#"+this.id).classed("click-highlight", true);

			})
			.on("dblclick",function(d,i){
				//d3.selectAll("."+"click-highlight").classed("click-highlight", false);
				//d3.select(this).classed("click-highlight", true);

				var is_current_to_noncurrent=(this.className.animVal.indexOf("current")!=-1);
				...
				if (is_current_to_noncurrent)
				{
					console.log(this.id)
					d3.selectAll("#"+this.id).classed("selected",false)
					//d3.selectAll("#"+this.id).classed("current",false)
					d3.selectAll("#"+this.id).classed("click-highlight",false)
				}
			});
		}
		...
		console.log("run changeComparedData function");
		changeComparedData(compareNum);
		if(dataCenter!= undefined){
			console.log(dataCenter);
			//转换排序标准时把原来的选中的柱子仍然选中
			if(dataCenter.datasets != undefined){
				for(var i = 0;i < dataCenter.datasets.length;i++){
					var treeId = dataCenter.datasets[i].id;
					chart.selectAll("#his-" + treeId).classed("selected", true);
				}
			}
		}
		...
		function changeComparedData(compareNum) {
			...
			if(!d3.selectAll("#his-" + compareNum).classed("selected")){
				chart.selectAll("#his-" + compareNum).classed("selected", true);
				...
				chart.selectAll("#his-" + compareNum).classed("selected", false);

				chart.select("#his-" + compareNum).classed("click-highlight",false);
	...
	*/
	function changePercentage(percentage, bg_index){
		var rectX = + chart.select("#his-" + bg_index).attr("x");
		var rectY = + chart.select("#his-" + bg_index).attr("y");
		var rectWidth = + chart.select("#his-" + bg_index).attr("width");
		var rectHeight = + chart.select("#his-" + bg_index).attr("height");
	    ...
	    if (message == "percentage") {
	    	if (datadimMode=="flowsize")
	    	{
	    		...
	    		if(data[0] != undefined && data[2] != undefined){
	    			changePercentage(data[0],data[2]);
	    		}
			}
			else if (datadimMode=="nodenum")
			{
